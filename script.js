...
// ==========================================================================
// SCHNELLE ABWESENHEIT POPUP (MODAL) LOGIK
// ==========================================================================
function oeffneAbwesenheitModal() {
    const selectKlasse = document.getElementById("modalKlassenAuswahl");
    selectKlasse.innerHTML = '<option value="">-- Klasse wählen --</option>';
    document.getElementById("modalSchuelerAuswahl").innerHTML = '<option value="">-- Erst Klasse wählen --</option>';
    
    // Befülle das Klassen-Dropdown aus der DB
    db.collection("klassen").get().then(snapshot => {
        snapshot.forEach(doc => {
            let opt = document.createElement("option");
            opt.value = doc.id;
            opt.innerText = "Klasse " + doc.id;
            selectKlasse.appendChild(opt);
        });
...
function modalLadeSchueler() {
    const klasse = document.getElementById("modalKlassenAuswahl").value;
    const selectSchueler = document.getElementById("modalSchuelerAuswahl");
    selectSchueler.innerHTML = '<option value="">-- Schüler wählen --</option>';
    ...
    db.collection("schueler").where("klasse", "==", klasse).get().then(snapshot => {
        snapshot.forEach(doc => {
            ...
            selectSchueler.appendChild(opt);
        });
    });
}
...
function speichereAbwesenheitAusModal() {
    const klasse = document.getElementById("modalKlassenAuswahl").value;
    const schueler = document.getElementById("modalSchuelerAuswahl").value;
    const status = document.getElementById("modalStatusAuswahl").value;
    const datum = document.getElementById("aktuellesDatum").value;

    if(!klasse || !schueler || !status) {
        return alert("Bitte fülle alle Felder aus!");
    }

    // Setze die Abwesenheit für 8 Stunden dieses Tages
    let promises = [];
    for(let std = 1; std <= 8; std++) {
        const key = `${klasse}_${datum}_Std${std}`;
        let updateObj = {};
        updateObj[`anwesenheit.${schueler}`] = status;
        
        let p = db.collection("klassenbuch").doc(key).update(updateObj).catch(() => {
            let initial = { anwesenheit: {} };
            initial.anwesenheit[schueler] = status;
            return db.collection("klassenbuch").doc(key).set(initial, { merge: true });
        });
        promises.push(p);
    }

    Promise.all(promises).then(() => {
        alert(`Status '${status}' für ${schueler} (Klasse ${klasse}) eingetragen.`);
        schliesseAbwesenheitModal();
        arbeitsbereichLadenAndRendern(); // Aktualisiert die Listen auf dem Dashboard
    });
}
